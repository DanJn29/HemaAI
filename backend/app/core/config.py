...
class Settings(BaseSettings):
    ...
    ocr_space_api_key: str | None = Field(default=None, validation_alias="OCR_SPACE_API_KEY", repr=False)
    ocr_space_endpoint: str = Field(
        default="https://api.ocr.space/parse/image",
        validation_alias="OCR_SPACE_ENDPOINT",
    )
    ocr_space_engine: int = Field(default=2, validation_alias="OCR_SPACE_ENGINE")
    ocr_space_timeout_seconds: int = Field(default=30, validation_alias="OCR_SPACE_TIMEOUT_SECONDS")
    ocr_max_file_size_mb: int = Field(default=5, validation_alias="OCR_MAX_FILE_SIZE_MB")
    ml_best_model_path: str = Field(
        default=str(PROJECT_ROOT / "artifacts" / "models" / "final_run" / "best_model.json"),
