...
def test_ml_predict_rejects_out_of_range_indicator_value_before_inference(client) -> None:
    payload = {
        **FULL_CBC_PAYLOAD,
        "values": {
            **FULL_CBC_PAYLOAD["values"],
            "WBC": 999,
        },
    }
    ...
    assert detail["errors"][0]["indicator_code"] == "WBC"
    assert detail["errors"][0]["provided_value"] == 999
    assert detail["errors"][0]["max_allowed"] == 300
